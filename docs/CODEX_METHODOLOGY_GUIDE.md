# CrushMe Codex Methodology Guide

CrushMe uses a **Codex-native repo runtime**. Claude Code is maintained as a compatibility layer only — it must stay aligned with the Codex-native source of truth, not redefine it.
...
| Compatibility only | `CLAUDE.md`, `.claude/` | Mixed-tool team support |
...
| `plugin` | Optional packaging format for sharing skills *across* repos (not CrushMe's daily runtime) |
...
4. Keep Claude Code materials aligned with the Codex-native source of truth — they must not redefine it.
...
## Compatibility Surface (Claude Code)

The `.claude/` directory is a **read-only mirror** of the Codex runtime. It must stay in sync but must not be the authoritative source.
...
When skills are added or modified, update both surfaces.
