...
- **Stack**: Django 5.2.17 LTS + DRF 3.18.0 (backend) / Vue 3.5 + Vite 7 SPA (frontend) / MySQL 8 / Redis / Huey
...
- **Resource limits**: MemoryMax=650M, CPUQuota=60%, OOMScoreAdjust=200
...
#### Dual auth strategy
- **Public API** (`/api/auth/...`) uses **JWT via SimpleJWT** with `ACCESS_TOKEN_LIFETIME=15m`, `REFRESH_TOKEN_LIFETIME=7d`, refresh rotation enabled.
