...
> **CrushMe override**: 100% of DRF views in `backend/crushme_app/*_views.py` are **function-based with `@api_view`** (auth, product, cart, order, wishlist, review, paypal, wompi, contact, etc.). Pattern: deserialize → service call → response. Never convert to CBV/APIView/ViewSets unless explicitly requested.
...
### Execution Rules
1. **Activate virtual environment** from inside `backend/`: `cd backend && source venv_cpu/bin/activate`. **The venv is `venv_cpu/`**, not `venv/` — it includes the PyTorch CPU build.
2. **Run only modified test files**: `pytest crushme_app/tests/path/to/test_file.py -v`. `pytest.ini` already sets `DJANGO_SETTINGS_MODULE`.
