...
Fake-data commands are allowed only when both the environment and database name
pass `crushme_app.fake_data_guard`. There is no production bypass.
...
| Test/E2E | `crushme_project.settings_test` | SQLite, in-memory cache/Huey, no external calls |
| Staging | `crushme_project.settings_staging` | Isolated MySQL/Redis; sandbox gateways |
